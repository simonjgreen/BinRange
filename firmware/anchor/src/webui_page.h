...
<div class=card>
  <div class=k style=margin-bottom:6px>MQTT <span id=m_pill class=pill></span></div>
  <div class=note id=m_broker>—</div>
  <div class=grid style=margin-top:10px>
    <div><div class=k>Published</div><div class=v id=m_pub>—</div></div>
    <div><div class=k>Failures</div><div class=v id=m_fail>—</div></div>
    <div><div class=k>Reconnects</div><div class=v id=m_rec>—</div></div>
    <div><div class=k>Last publish</div><div class=v id=m_age>—</div></div>
  </div>
  <div class=note id=m_err style=margin-top:8px></div>
  <div class=k style="margin:12px 0 4px">Recent traffic</div>
  <pre id=m_log style="margin:0;max-height:150px;overflow:auto;font:11px/1.5 ui-monospace,monospace;color:var(--dim);background:#0f1115;border-radius:6px;padding:8px">—</pre>

  <div class=k style="margin:14px 0 4px">Broker settings</div>
  <div class=row>
    <div><label>Host</label><input id=m_host style=width:130px></div>
    <div><label>Port</label><input id=m_port type=number style=width:70px></div>
    <div><label>Username</label><input id=m_user style=width:110px></div>
  </div>
  <div class=row style=margin-top:8px>
    <div><label>Password</label><input id=m_pass type=password style=width:130px></div>
    <div><label>Client ID</label><input id=m_cid style=width:150px></div>
    <button onclick=saveMqtt()>Apply and reconnect</button>
  </div>
  <div class=note>Leave the password blank to keep the stored one. It is never
  sent back by the API, so this page is safe to screenshot.</div>
</div>
...
<div class=card>
  ...
  <form id=anchorUpload method=POST action=/update enctype=multipart/form-data>
    <label for=anchorImage>Anchor firmware BIN</label>
    <input id=anchorImage type=file name=f accept=.bin required>
    <button id=anchorUploadButton type=submit>Upload and restart anchor</button>
  </form>
  <div class=note>Ranging halts during the update and the board reboots afterwards.</div>
  <div id=anchorUploadStatus class=note role=status aria-live=polite></div>
</div>
</div>
...
// Authenticate first so the file callbacks receive Digest and the current
// per-boot CSRF token. Never automatically replay a firmware POST on failure.
$('anchorUpload').addEventListener('submit',async event=>{
  event.preventDefault();
  const button=$('anchorUploadButton'), status=$('anchorUploadStatus');
  if(button.disabled)return;
  if(!confirm('Install this firmware on the anchor and restart it?'))return;
  button.disabled=true;
  const abort=new AbortController(), timeout=setTimeout(()=>abort.abort(),120000);
  let posted=false;
  try{
    status.textContent='Authenticating with the anchor admin password…';
    const admin=await fetch('/api/admin',{credentials:'same-origin',cache:'no-store',signal:abort.signal});
    ...
    status.textContent='Uploading anchor firmware…';
    posted=true;
    const response=await fetch('/update',{method:'POST',credentials:'same-origin',
      headers:{'X-BinRange-CSRF':auth.csrf},body:new FormData($('anchorUpload')),signal:abort.signal});
    const result=await response.text();
    if(!response.ok)throw Error('Upload rejected ('+response.status+'): '+result);
...
async function tick(){
  ...
  // Under DS-TWR the anchor computes the range, so it always has a distance.
...
async function mtick(){
  let r; try{ r=await (await fetch('/api/mqtt')).json(); }catch(e){ return; }
  const p=$('m_pill');
  p.textContent = r.connected ? 'connected' : 'disconnected';
  p.className = 'pill ' + (r.connected ? 'on' : 'off');
  $('m_broker').textContent = r.broker;
  $('m_pub').textContent=r.published; $('m_fail').textContent=r.failed;
  $('m_rec').textContent=r.reconnects;
  $('m_age').textContent = r.last_pub_age==null ? '—'
      : (r.last_pub_age/1000).toFixed(1)+' s ago';
  $('m_err').textContent = (!r.connected || r.failed)
      ? 'Last result: '+r.last_rc_text+' ('+r.last_rc+')' : '';
  $('m_log').textContent = r.log.length
      ? r.log.slice().reverse().map(e=>
          (e.age/1000).toFixed(1).padStart(6)+'s  '+e.t).join('\n')
      : 'nothing published yet';
  if(!mtouched){
    $('m_host').value=r.host; $('m_port').value=r.port;
    $('m_user').value=r.user; $('m_cid').value=r.client_id;
    $('m_pass').placeholder = r.pass_set ? 'unchanged' : 'not set';
  }
}
