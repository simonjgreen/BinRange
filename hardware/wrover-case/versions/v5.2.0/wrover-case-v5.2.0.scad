// Makerfabs ESP32 UWB DW3000 / WROVER-E enclosure, V5.2 TRIAL. All units mm.
...
// Full-wall-depth hook experiment. V5 base fits when closed. Test coupon first.
// Larger deflection and PCB clearance during release are NOT validated.
...
clip_thickness = 1;
clip_side_gap = 1;
clip_engagement = wall; // TRIAL: 3 mm engagement, nominally flush with base exterior.
...
assert(clip_engagement >= 0.5 && clip_engagement <= wall);
...
echo(str("V5.2 full-wall-hook TRIAL: ",outer_l," x ",outer_w," x ",total_h," mm"));
echo(str("Four PCB springs: free preload ",board_preload,
         "; nominal preload at latch stop ",board_preload-clip_lift_clearance," mm"));
echo("Both parts are in print orientation; Trial lid: unchanged V5 base; test closure sample without PCB first.");
