...
include <wrover-case-v5.2.0.scad>
check = "closed";
...
if(check=="release") for(lift=[0:1:16]) intersection() {
    ...
    tongues_assembled(lift,clip_engagement+0.5);
}
...
// Same conservative release envelope, without the PCB.
if(check=="release_empty_base") for(lift=[0:1:16]) intersection() {
    base(); tongues_assembled(lift,clip_engagement+0.5);
}
